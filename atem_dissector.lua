...
pf_fields["pf_field_vumeter_opacity"] = ProtoField.new  ("VuMeter Opacity", "atem.field.vumeter.opacity", ftypes.UINT8, nil, base.DEC)
...
pf_fields["pf_field_afv"]   	= ProtoField.new  ("Follow Video", "atem.field.afv", ftypes.BOOLEAN, {"On", "Off"}, 1, 0x1)
...
VALS["VALS_CAMM_SETMASK25"] = {[0] = "Volume", [1] = "Balance", [2] = "Follow Video"}
...
pf_fields["pf_cmd_ramp_setmask27"]  = ProtoField.new  ("Set Mask", "atem.cmd.ramp.setmask27", ftypes.UINT8, nil, base.DEC, 0xf)
pf_fields["pf_flag_cmd_ramp_setmask27_all"] = ProtoField.new ("All", "atem.cmd.ramp.setmask27.flags.all", ftypes.BOOLEAN, {"On","Off"}, 4, 0x1)
...
cmd_labels["VuMo"] = "Multi Viewer VuMeter Opacity"
...
function atem_proto.dissector(tvbuf,pktinfo,root)
	...
	while (pktlen_remaining > 0) do
		...
		elseif (cmd_name == "CMvI") then
			...
			cmd_tree:add(pf_fields["pf_field_windowindex"], tvbuf:range(pos+9, 1))
			cmd_tree:add(pf_fields["pf_field_videosource"], tvbuf:range(pos+10, 2))
		elseif (cmd_name == "VuMo") then
			cmd_tree:add(pf_fields["pf_field_multiviewer"], tvbuf:range(pos+8, 1))
			cmd_tree:add(pf_fields["pf_field_vumeter_opacity"], tvbuf:range(pos+9, 1))
		...
		elseif (cmd_name == "AMIP") then
			...
			cmd_tree:add(pf_fields["pf_field_unknown3"], tvbuf:range(pos+22, 4))
		...
		elseif (cmd_name == "AMMO") then
			...
			cmd_tree:add(pf_fields["pf_field_balance"], tvbuf:range(pos+10, 2))
			cmd_tree:add(pf_fields["pf_field_afv"], tvbuf:range(pos+12, 1))
			cmd_tree:add(pf_fields["pf_cmd_ammo_unknown9"], tvbuf:range(pos+13, 6))
		elseif (cmd_name == "CAMM") then
			...
			cmd_tree:add(pf_fields["pf_field_balance"], tvbuf:range(pos+12, 2))
			cmd_tree:add(pf_fields["pf_field_afv"], tvbuf:range(pos+14, 1))
		elseif (cmd_name == "AMmO") then
		...
		elseif (cmd_name == "RAMP") then
			...
			cmd_ramp_setmask27_tree:add(pf_fields["pf_flag_cmd_ramp_setmask27_all"], tvbuf:range(pos+8, 1))
			...
			cmd_ramp_setmask27_tree:add(pf_fields["pf_flag_cmd_ramp_setmask27_monitor"], tvbuf:range(pos+8, 1))
